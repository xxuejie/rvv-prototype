...
pub struct HacspecSpan(pub Span);

impl Serialize for HacspecSpan {
...
pub type Spanned<T> = (T, HacspecSpan);

impl From<Span> for HacspecSpan {
    fn from(x: Span) -> HacspecSpan {
        HacspecSpan(x)
    }
}
